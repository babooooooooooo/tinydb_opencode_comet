...
tinydb v0.2挑战。在v0.1版本的基础上，
实现：
1、多表 JOIN 查询
2、并发控制（多线程/多进程安全）
3、CLI功能增强，多行，语法颜色，行编辑，新命令，如.explain进行SQL执行计划解析等。

所以开发不允许从头开发，必须在v0.1版本上进行变更。v0.2发布时，必须spec进行完整合并。 以上功能开发，尽可能使用git worktree隔离后并行开发。